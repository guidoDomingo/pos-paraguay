    ...
    <tr>
            ...
            <table style="width:100%; border-collapse:collapse;">
                <tr>
                    ...
                    <td style="vertical-align:top;">
                        ...
                        @if($companyActivity)
                            <div class="company-sub">{{ $companyActivity }}</div>
                        @endif
                        @if($companyPhone)
                            <div class="company-sub">Cel.: {{ $companyPhone }}</div>
                        @endif
                        @if($companyAddress)
        ...
        <td class="header-right">
            @if($invoice->fiscalStamp)
                ...
                <div class="ruc-line">RUC: {{ $companyRuc }}</div>
                <div class="factura-title">FACTURA</div>
                <div class="vigencia-line">Fecha Inicio Vigencia: {{ $invoice->fiscalStamp->valid_from->format('d/m/Y') }}</div>
                <div class="vigencia-line">Válido Hasta: {{ $invoice->fiscalStamp->valid_until->format('d/m/Y') }}</div>
            @else
                <div class="ruc-line">RUC: {{ $companyRuc }}</div>
                <div class="factura-title">FACTURA</div>
            @endif
...
<table class="client-table">
    <tr>
        <td style="width:55%;">
            Fecha de Emisión: <strong>{{ $fecha->day }}</strong> de <strong>{{ $mesNombre }}</strong> de <strong>{{ $fecha->year }}</strong>
        </td>
        <td style="width:45%;">
            Cond. de Venta:
            <span class="check-box">{{ $esContado ? 'X' : '' }}</span> Contado &nbsp;
            <span class="check-box">{{ $esCredito ? 'X' : '' }}</span> Crédito
        </td>
    </tr>
    <tr>
        <td>Nombre o Razón Social: <strong>{{ $invoice->customer_name ?? '' }}</strong></td>
        <td>Nota de Remisión Nº:</td>
    </tr>
    <tr>
        <td>R.U.C.: <strong>{{ $invoice->customer_ruc ?? '' }}</strong></td>
        <td>Teléfono:</td>
    </tr>
    <tr>
        <td colspan="2">Dirección: <strong>{{ $invoice->customer_address ?? '' }}</strong></td>
    </tr>
</table>
...
<table class="items-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="col-pu">{{ number_format($item->unit_price, 0, ',', '.') }}</td>
            <td class="col-exent">{{ $item->iva_type === 'EXENTO' ? number_format($item->total_price, 0, ',', '.') : '' }}</td>
            <td class="col-5">{{ $item->iva_type === 'IVA_5'   ? number_format($item->total_price, 0, ',', '.') : '' }}</td>
            <td class="col-10">{{ $item->iva_type === 'IVA_10'  ? number_format($item->total_price, 0, ',', '.') : '' }}</td>
        </tr>
        @endforeach
...
<table class="totals-table">
    <tr>
        ...
        <td class="amount-cell">{{ number_format($valorParcial, 0, ',', '.') }}</td>
    </tr>
    <tr>
        <td class="label-cell" style="font-size:10px;">TOTAL A PAGAR Gs.</td>
        <td class="amount-cell" style="font-size:10px; font-weight:bold;">{{ number_format($invoice->total_amount, 0, ',', '.') }}</td>
    </tr>
    <tr>
        <td colspan="2" style="padding:2px 5px; font-size:7.5px; border:1px solid #000;">
            SON: <strong>{{ $totalLetras }}</strong>
        </td>
    </tr>
</table>
...
<table class="iva-footer">
    <tr>
        <td style="width:50%;">
            TOTAL IVA: &nbsp; <strong>{{ number_format($invoice->total_iva ?? 0, 0, ',', '.') }}</strong>
        </td>
    ...
    <tr>
        <td colspan="2">
            Liquidación del IVA:&nbsp;&nbsp;
            (5 %) <strong>{{ number_format($invoice->total_iva_5 ?? 0, 0, ',', '.') }}</strong>
            &nbsp;&nbsp;&nbsp;
            (10 %) <strong>{{ number_format($invoice->total_iva_10 ?? 0, 0, ',', '.') }}</strong>
        </td>
    </tr>
</table>
