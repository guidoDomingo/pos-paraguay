...
<div class="center" style="margin-bottom:4px;">
    <img src="{{ $logoPath }}" style="max-height:18mm; max-width:40mm;">
</div>
@endif
...
<div class="center">
    ...
    @if($companyActivity)<div class="co-sub">{{ $companyActivity }}</div>@endif
    @if($companyPhone)<div class="co-sub">Tel: {{ $companyPhone }}</div>@endif
...
<div class="center stamp-section">
    @if($invoice->fiscalStamp)
        ...
        <div class="stamp-num">{{ $invoice->fiscalStamp->stamp_number }}</div>
        <div class="co-sub">RUC: {{ $companyRuc }}</div>
        <div class="factura-title">FACTURA</div>
        <div class="co-sub" style="font-size:8px;">
            Vigencia: {{ $invoice->fiscalStamp->valid_from->format('d/m/Y') }} al {{ $invoice->fiscalStamp->valid_until->format('d/m/Y') }}
        </div>
    @else
        <div class="co-sub">RUC: {{ $companyRuc }}</div>
        <div class="factura-title">FACTURA</div>
...
<div class="client-section">
    <div class="client-row">
        ...
        <span class="bold">{{ $fecha->day }} de {{ $mesNombre }} de {{ $fecha->year }}</span>
    </div>
    <div class="client-row">
        <span class="field-lbl">Cond.:</span>
        <span class="bold">{{ $esContado ? 'CONTADO' : 'CRÉDITO' }}</span>
    </div>
    <div class="client-row">
        <span class="field-lbl">Cliente:</span>
        <span class="bold">{{ $invoice->customer_name ?? 'Consumidor Final' }}</span>
    </div>
    @if($invoice->customer_ruc)
    <div class="client-row">
        <span class="field-lbl">RUC:</span>
        <span class="bold">{{ $invoice->customer_ruc }}</span>
    </div>
    @endif
...
<div class="item-row">
    ...
    <div class="item-line">
        <div class="item-left">
            ...
            <span class="iva-tag">
                @if($item->iva_type === 'IVA_10') (10%)
                @elseif($item->iva_type === 'IVA_5') (5%)
                @else (Exento)
                @endif
            </span>
        </div>
        <div class="item-right">Gs. {{ number_format($item->total_price, 0, ',', '.') }}</div>
    </div>
</div>
...
<div class="total-line">
    ...
    <div class="tl-value">Gs. {{ number_format($valorParcial, 0, ',', '.') }}</div>
</div>
<div class="dashed"></div>
<div class="total-line total-final">
    <div class="tl-label">TOTAL:</div>
    <div class="tl-value">Gs. {{ number_format($invoice->total_amount, 0, ',', '.') }}</div>
</div>
<div class="dashed"></div>

{{-- ── SON ── --}}
<div class="son-section">SON: {{ $totalLetras }}</div>

<div class="dashed"></div>

{{-- ── IVA ── --}}
<div class="iva-section">
    <div>TOTAL IVA: <span class="bold">Gs. {{ number_format($invoice->total_iva ?? 0, 0, ',', '.') }}</span></div>
    <div>
        Liq. IVA: (5%) Gs. <span class="bold">{{ number_format($invoice->total_iva_5 ?? 0, 0, ',', '.') }}</span>
        &nbsp; (10%) Gs. <span class="bold">{{ number_format($invoice->total_iva_10 ?? 0, 0, ',', '.') }}</span>
    </div>
</div>
...
<div class="footer-section">
    ...
    @if($settings->footer_text ?? null)
        <div style="margin-top:3px;">{{ $settings->footer_text }}</div>
    @endif
    <div style="margin-top:4px; font-size:7.5px;">{{ $fecha->format('d/m/Y H:i') }}</div>
</div>
