    ...
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="h4 font-weight-bold mb-0">
                <i class="bi bi-sliders me-2"></i>{{ __('Ajustar Stock') }}
            </h2>
            ...
            <div class="row justify-content-center">
                <div class="col-xl-8">
                    <div class="card shadow">
                        ...
                        <div class="card-body">
                            <form method="POST" action="{{ route('inventory.adjust.store') }}" id="adjustmentForm">
                                @csrf
                                            ...
                                            <select class="form-select @error('product_id') is-invalid @enderror" id="product_id" name="product_id" required>
                                                ...
                                                @foreach($products as $product)
                                                    <option value="{{ $product->id }}" 
                                                            ...
                                                            data-unit="{{ $product->unit ?? 'unidades' }}"
                                                            data-min-stock="{{ $product->min_stock ?? 0 }}"
                                                            data-cost="{{ $product->cost_price ?? 0 }}"
                                                            {{ request('product') == $product->id || old('product_id') == $product->id ? 'selected' : '' }}>
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select @error('adjustment_type') is-invalid @enderror" id="adjustment_type" name="adjustment_type" required>
                                                ...
                                                <option value="add" {{ old('adjustment_type') == 'add' ? 'selected' : '' }}>Agregar Stock</option>
                                                <option value="subtract" {{ old('adjustment_type') == 'subtract' ? 'selected' : '' }}>Reducir Stock</option>
                                                <option value="set" {{ old('adjustment_type') == 'set' ? 'selected' : '' }}>Establecer Stock</option>
                                            </select>
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea class="form-control @error('reason') is-invalid @enderror" 
                                              ...
                                              placeholder="Describe el motivo del ajuste de inventario...">{{ old('reason') }}</textarea>
                                    @error('reason')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
