            ...
            <div class="row justify-content-center">
                <div class="col-xl-7 col-lg-9">
                    ...
                    <div class="d-flex align-items-center gap-3 mb-4">
                        <a href="{{ route('cash.current') }}" class="btn btn-outline-secondary btn-sm">
                    ...
                    <div class="row g-4">
                                ...
                                <div class="card-body">
                                    <table class="table table-sm mb-0">
                                        <tr>
                                            ...
                                            <td class="text-end fw-semibold">₲ {{ number_format($register->opening_amount, 0, ',', '.') }}</td>
                                        </tr>
                                        <tr>
                                            <td class="text-muted"><i class="bi bi-cash me-1 text-success"></i>Ventas efectivo</td>
                                            <td class="text-end text-success fw-semibold">+ ₲ {{ number_format($byMethod['CASH'], 0, ',', '.') }}</td>
                                        </tr>
                                        <tr>
                                            <td class="text-muted"><i class="bi bi-credit-card me-1 text-primary"></i>Tarjeta</td>
                                            <td class="text-end fw-semibold">₲ {{ number_format($byMethod['CARD'], 0, ',', '.') }}</td>
                                        </tr>
                                        <tr>
                                            <td class="text-muted"><i class="bi bi-bank me-1 text-info"></i>Transferencia</td>
                                            <td class="text-end fw-semibold">₲ {{ number_format($byMethod['TRANSFER'], 0, ',', '.') }}</td>
                                        </tr>
                                        @if($byMethod['CHEQUE'] > 0)
                                        <tr>
                                            <td class="text-muted"><i class="bi bi-journal-check me-1"></i>Cheque</td>
                                            <td class="text-end fw-semibold">₲ {{ number_format($byMethod['CHEQUE'], 0, ',', '.') }}</td>
                                        </tr>
                                        @endif
                                        <tr>
                                            <td class="text-muted"><i class="bi bi-plus-circle me-1 text-success"></i>Ingresos manuales</td>
                                            <td class="text-end text-success fw-semibold">+ ₲ {{ number_format($incomes, 0, ',', '.') }}</td>
                                        </tr>
                                        <tr>
                                            <td class="text-muted"><i class="bi bi-dash-circle me-1 text-danger"></i>Egresos manuales</td>
                                            <td class="text-end text-danger fw-semibold">- ₲ {{ number_format($expenses, 0, ',', '.') }}</td>
                                        </tr>
                                        <tr class="table-light fw-bold border-top">
                                            <td>💵 Esperado en caja</td>
                                            <td class="text-end text-success fs-5">₲ {{ number_format($expected, 0, ',', '.') }}</td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="card border-0 shadow-sm h-100" style="border-radius:15px;">
                                ...
                                <div class="card-body">
                                    <form method="POST" action="{{ route('cash.close.store') }}" id="closeForm">
                                        @csrf
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <div class="input-group input-group-lg">
                                                ...
                                                <input type="number" name="closing_amount" id="closingAmount" class="form-control fw-bold"
                                                       ...
                                                       value="{{ old('closing_amount') }}" required autofocus
                                                       oninput="calcDiff()">
                                            </div>
                                        </div>
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <textarea name="closing_notes" class="form-control" rows="3"
                                                      placeholder="Observaciones del cierre...">{{ old('closing_notes') }}</textarea>
                                        </div>
                                        ...
                                        <div class="d-grid gap-2">
                                            ...
                                            <a href="{{ route('cash.current') }}" class="btn btn-outline-secondary">Cancelar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
