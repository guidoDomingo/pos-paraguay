    ...
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="h4 font-weight-bold mb-0">
                <i class="bi bi-pencil me-2"></i>{{ __('Editar Categoría') }}
            </h2>
            <div class="btn-group">
                            ...
                            <form method="POST" action="{{ route('categories.update', $category) }}">
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                ...
                                                <br><span class="text-muted" id="preview-description">{{ $category->description ?: 'Sin descripción' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                @if($category->products->count() > 0)
                                    <div class="alert alert-info">
                                        ...
                                        <strong>Productos asociados:</strong> Esta categoría tiene {{ $category->products->count() }} producto(s) asignado(s).
                                        <ul class="mb-0 mt-2">
                                            @foreach($category->products->take(5) as $product)
                                                <li>{{ $product->name }}</li>
                                            @endforeach
                                            @if($category->products->count() > 5)
                                                <li><em>... y {{ $category->products->count() - 5 }} más</em></li>
                                            @endif
                                        </ul>
                                    </div>
                                @else
                                    <div class="alert alert-warning">
                                        ...
                                        <strong>Categoría vacía:</strong> Esta categoría no tiene productos asignados aún.
                                    </div>
                                @endif
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <p class="mb-0">{{ $category->updated_at->format('d/m/Y H:i:s') }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
